  ...
  return (
    ...
    <div className="fixed top-0 left-0 w-screen bg-white p-6 tablet:bg-slate-300">
      ...
      <div className="mt-36 tablet:mt-44">
          {filtermc.map((mc) => (
            <div key={mc._id}>
              <img src={mc.picture} alt="" className="h-40 w-[50%] rounded-2xl tablet:w-80 tablet:h-52"/>
              <p className="font-semibold text-[15px] text-slate-800 tablet:-[20px]">{mc.name}</p>
            </div>
          ))}
      </div>
